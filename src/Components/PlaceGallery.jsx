...
const PlaceGallery = ({places}) => {
        ...
        return(
            <div className='absolute inset-0 bg-black text-white min-h-screen '>   
                <div className='bg-black p-8 grid gap-4'>
                    ...
                    {places?.photos?.length > 0 && places.photos.map(photo => (
                    <div>
  ...
  return (
      ...
      <div className='relative'>
            ...
            <div className='grid'>
                {places.photos?.[1] && (
                    <img onClick={() => setShowAllPhotos(true)} className='cursor-pointer aspect-square object-cover' src={`http://localhost:4000/uploads/${places.photos?.[1]}`} alt=''/>
                )}
            <div className='overflow-hidden'>
                {places.photos?.[2] && (
                    <img onClick={() => setShowAllPhotos(true)} className='cursor-pointer aspect-square object-cover realtive top-2' src={`http://localhost:4000/uploads/${places.photos?.[2]}`} alt=''/>
                )}
            </div>
